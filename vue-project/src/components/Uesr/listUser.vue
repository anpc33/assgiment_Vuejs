...
export default {
  setup() {
    ...
    const getData = async (page = 1) => {
      try {
        const response = await userApi.getPaginatedUsers(page, limit.value);
    ...
    const deleteUser = async (id) => {
      ...
      if (confirmed) {
        try {
          await userApi.deleteUser(id); // Gọi API xóa
  ...
  <div class="bg-white p-8 rounded-md w-full" style="min-height: 100vh">
    <div class="flex items-center justify-between pb-6">
      <div>
        ...
        <span class="text-xs" style="color: black">All User item</span>
      </div>
      <div class="flex items-center justify-between">
        <div class="flex bg-gray-50 items-center p-2 rounded-md"></div>
        <div class="lg:ml-40 ml-10 space-x-8 mr-4">
          <RouterLink to="/create" class="text-sm" style="color: black"
            ><svg
      ...
      <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
        <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
          <table class="min-w-full leading-normal">
            ...
            <tbody>
              <tr v-for="(userItem, index) in data" :key="index">
                ...
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <div class="flex justify-center gap-3 items-center">
                    <div>
                      <button
                        @click="deleteCategory(userItem.id)"
