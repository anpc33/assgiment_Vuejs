...
export default {
  setup() {
    ...
    const getData = async (page = 1) => {
      try {
        const response = await CategoryApi.getPaginatedcategory(
          page,
          limit.value
        );
    ...
    const deleteCategory = async (id) => {
      ...
      if (confirmed) {
        try {
          await CategoryApi.deleteCategory(id); // Gọi API xóa
...
<template>
  ...
  <div class="bg-white p-8 rounded-md w-full" style="min-height: 100vh">
    <div class="flex items-center justify-between pb-6">
      <div>
        <h2 class="text-gray-600 font-semibold">Danh mục</h2>
        <span class="text-xs" style="color: black">Tất cả danh mục</span>
      </div>
      ...
      <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
        <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
          <table class="min-w-full leading-normal">
            ...
            <tbody>
              <tr v-for="(categoryItem, index) in data" :key="index">
                ...
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <div class="flex justify-center gap-3 items-center " >
                    ...
                    <div>
                      <RouterLink
                        :to="`/updateCategory/${categoryItem.id}`"
                        class="text-sm"
                      >
                        <svg
