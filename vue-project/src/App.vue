...
import Sidebar from "./components/layout/sidenav.vue"; // Đường dẫn đến Sidebar

// Reactive variables
const isLoggedIn = ref(localStorage.getItem("isLoggedIn") === "true");
const userEmail = ref("");
const userRole = ref(null); // Thêm biến để lưu role của user
const router = useRouter();
const isDropdownVisible = ref(false); // Trạng thái hiển thị của menu dropdown
...
// Đăng xuất
const logout = () => {
  clearLocalStorage(); // Xóa tất cả dữ liệu trong localStorage
  isLoggedIn.value = false;
  userRole.value = null;
  router.push({ name: "login" });
};

// Hàm clear dữ liệu localStorage
...
const updateLoginState = () => {
  ...
  // Kiểm tra nếu không đăng nhập hoặc role không phải 1, xóa dữ liệu và điều hướng
  if (!loggedIn || role !== "1") {
    clearLocalStorage();
    isLoggedIn.value = false;
    userRole.value = null;
...
onMounted(() => {
  // Cập nhật trạng thái lần đầu khi App được mount
  updateLoginState();

  // Lắng nghe sự kiện thay đổi localStorage từ các tab hoặc cửa sổ khác
  window.addEventListener("storage", updateLoginState);
});

onUnmounted(() => {
  // Loại bỏ sự kiện khi component bị destroy
...
<template>
  <div class="" style="min-height: 100vh">
    ...
    <div v-if="isLoggedIn && userRole == '1'"  class="flex w-full">
      <div class="" style="min-height: 100vh">
        <Sidebar v-if="isLoggedIn && userRole === '1'" />
      </div>

      <div v-if="isLoggedIn && userRole === '1'" style="display: flex; width: 100%">
        <!-- <Sidebar /> -->
        <div
          class=""
          style="
        ...
        >
          <!-- navbar -->
          <div
            class="py-2 px-6 bg-[#f8f4f3] flex items-center shadow-md shadow-black/5"
          >
            <button
              type="button"
            ...
            <ul class="ml-auto flex items-center">
              ...
              <li class="dropdown ml-3 relative">
                  ...
                  <div class="flex-shrink-0 w-10 h-10 relative">
                    ...
                    >
                      <img
                        ...
                        src="https://i.imgur.com/Egv10oM.jpeg"
                        alt="User Avatar"
                      />
                      <!-- Status Indicator -->
                      <div
                ...
                <ul
                  ...
                  class="dropdown-menu shadow-md z-30 py-1.5 rounded-md bg-white border border-gray-100 w-full max-w-[140px] absolute right-0 mt-2"
                >
                  <li>
                    <a
                      href="#"
    ...
    <!-- Main content -->
    <div v-if="!isLoggedIn && userRole != '1'" class="w-full" >
...
/*
1.  the user's configured `mono` font family by default.
